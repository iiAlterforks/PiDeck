...
| **历史参考实现** | 已归档/可删除 | 早期 Pi CLI 飞书扩展参考，不参与 PiDeck 运行 |

> **实际运行的是 PiDeck（pi-desktop）**。当前飞书功能由 `src/main/feishu/` 和官方 `@larksuiteoapi/node-sdk` 提供，历史参考实现不再作为运行时依赖。
