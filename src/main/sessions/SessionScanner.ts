...
export class SessionScanner {
  ...
  private async collectJsonl(dir: string): Promise<string[]> {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) files.push(...await this.collectJsonl(path));
        ...
        // pi-interactive-subagents writes child sessions beside their parent session.
        // Their generated file id is not a standard UUID, so exclude them from the
        // top-level history list while retaining normal and imported sessions.
  ...
  private isSubagentSessionFile(fileName: string): boolean {
    ...
    return !standardUuid.test(idPart);
  }
